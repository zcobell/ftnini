// INIReader.h sourced from https://github.com/jtilly/inih
// 
// INIReader.h has been modified for this setting
// 
// (1) Use const string references 
// (2) Use unordered_map and unordered_set
// (3) Added a function to check if a key exists
// 
// --Zach Cobell
...
class INIReader {
 ...
 protected:
  ...
  std::unordered_set<std::string> _sections;
...
inline const std::unordered_set<std::string>& INIReader::Sections() const {
  return _sections;
}
...
inline bool INIReader::contains(const std::string& section,
                                const std::string& name) {
  std::string key = MakeKey(section, name);
